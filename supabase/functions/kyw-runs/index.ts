...
// ?date=YYYY-MM-DD pulls that date. ?next=1 works kyw_runs_queue newest-first until the time budget (?budget= ms, default 110000) is spent.
...
async function pullDate(date: string) {
  ...
  // Three meetings at a time; races within a meeting in parallel.
  for (let i = 0; i < meets.length; i += 3) {
    await Promise.all(meets.slice(i, i + 3).map(async (m: any) => {
      try {
        ...
        const all = (await Promise.all(nos.map(async (n) => { try { const x = await gql(RACE_Q(String(m.id), n)); return mapRace({ ...m, date: m.date ?? date }, x.r ?? {}); } catch (e) { out.push({ meet: m.venueName, race: n, error: String(e).slice(0, 120) }); return []; } }))).flat();
        races += nos.length;
      ...
      } catch (e) { out.push({ meet: m.venueName, error: String(e).slice(0, 200) }); }
    }));
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Work the queue, newest date first, until the time budget is spent (Edge wall clock is limited).
    const budget = Math.min(140000, +(url.searchParams.get("budget") ?? 110000));
    const done: any[] = [];
    while (Date.now() - t0 < budget) {
      ...
      try {
        ...
        await sb.from("kyw_runs_queue").update({ status: "failed", finished_at: new Date().toISOString(), errors: 1, ms: Date.now() - t1 }).eq("date", d);
